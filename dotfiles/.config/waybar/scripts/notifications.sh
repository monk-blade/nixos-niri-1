...
get_notification_count() {
    makoctl list 2>/dev/null | jq -r '.data[0] | length' 2>/dev/null || echo "0"
}

# Show notification history with fuzzel
show_notifications() {
    # Get notifications from mako
    notifications=$(makoctl list 2>/dev/null | jq -r '.data[0][]? | "\(.summary.data // "No Title"): \(.body.data // "No Content")"' 2>/dev/null)
...
$notifications"
    ...
    case "$selected" in
        ...
        "📋 Notification History")
            ...
            makoctl list 2>/dev/null | jq -r '.data[0][]? | "Title: \(.summary.data // "No Title")\nBody: \(.body.data // "No Content")\nApp: \(.app_name.data // "Unknown")\nTime: \(.timestamp.data // "Unknown")\n───────────────────────"' 2>/dev/null | fuzzel --dmenu --prompt="History: " --width=80 --lines=15
            ;;
    esac
}
